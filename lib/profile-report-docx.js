...
function splitBulletLines(text, sizeHalf, opts) {
  opts = opts || {};
  const prefix = opts.prefix != null ? opts.prefix : "     ·"; // 5 spaces + bullet
  const cont = opts.cont != null ? opts.cont : "       "; // line 2 indent
  const budget = opts.budget || BODY_BUDGET;
...
// Summary table: fixed columns (성명 3cm / 주요경력 및 학력 7cm / 전문역량 7cm) and
// every glyph condensed to 장평 90%.
const SUM_NAME_W = 1701; // 3cm
const SUM_MID_W = 3969; // 7cm
const SUM_COMP_W = 3968; // 7cm (sums with the others to the usable width)
const SUM_SCALE = 90;

// Most recent degree first.
function eduYearNum(e) {
  const m = String((e && e.year) || "").match(/(\d{2,4})/);
  if (!m) return -1;
  let y = Number(m[1]);
...
function summaryNameCell(data) {
  const kids = [];
  if (data.category) kids.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 30, line: LINE_TIGHT, lineRule: "auto" }, children: [run("【 " + cleanText(data.category) + " 】", { size: SZ_TABLE_SMALL, bold: true, scale: SUM_SCALE })] }));
  ...
  kids.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 0, line: LINE_TIGHT, lineRule: "auto" }, children: [run(data.name || "", { size: SZ_TABLE, bold: true, scale: SUM_SCALE })] }));
  if (data.birthLine) kids.push(birthLineParagraph(data.birthLine));
  return new TableCell({ verticalAlign: VerticalAlign.CENTER, margins: { top: 40, bottom: 40, left: 40, right: 40 }, children: kids });
}

function summaryCareerCell(data) {
  // Careers first (last line gets a 10pt gap to separate from 학력), then
  // education newest-first. All at 장평 90%.
  const edu = (data.education || []).slice().sort((a, b) => eduYearNum(b) - eduYearNum(a));
  const kids = careerLines(data.career, { size: SZ_TABLE, scale: SUM_SCALE, lastAfter: 200 });
  educationLines(edu, { size: SZ_TABLE, scale: SUM_SCALE, lastAfter: 0 }).forEach((p) => kids.push(p));
...
function summaryCompetencyCell(data) {
  const kids = [];
  const budget = Math.round((SUM_COMP_W - 140) / (SUM_SCALE / 100)); // account for 90% scale
  // 한 줄 요약: 뒷단락 6pt.
  if (data.competencyTitle) kids.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 120, line: LINE_TIGHT, lineRule: "auto" }, children: [run(cleanText(data.competencyTitle), { size: SZ_TABLE, bold: true, underline: true, scale: SUM_SCALE })] }));
  const bullets = [];
  ...
  bullets.forEach((t) => {
    // "·" immediately followed by text; line 2 indented 2 spaces to align under
    // line 1. Line 1 hugs line 2 (0pt), line 2 gets 뒷단락 6pt.
    const lines = splitBulletLines(cleanText(stripLead(t)), SZ_TABLE, { budget: budget, prefix: "·", cont: "  " });
    if (lines.length === 1) {
      kids.push(new Paragraph({ spacing: { before: 0, after: 120, line: LINE_TIGHT, lineRule: "auto" }, children: [run(lines[0], { size: SZ_TABLE, scale: SUM_SCALE })] }));
    } else {
      kids.push(new Paragraph({ spacing: { before: 0, after: 0, line: LINE_TIGHT, lineRule: "auto" }, children: [run(lines[0], { size: SZ_TABLE, scale: SUM_SCALE })] }));
      kids.push(new Paragraph({ spacing: { before: 0, after: 120, line: LINE_TIGHT, lineRule: "auto" }, children: [run(lines[1], { size: SZ_TABLE, scale: SUM_SCALE })] }));
    }
  });
  // 마지막 ※ 줄: 뒷단락 0pt.
  if (data.reviewNote) kids.push(new Paragraph({ spacing: { before: 0, after: 0, line: LINE_TIGHT, lineRule: "auto" }, children: [run("※ " + cleanText(data.reviewNote), { size: SZ_TABLE, scale: SUM_SCALE })] }));
  if (!kids.length) kids.push(new Paragraph({ children: [run("", { size: SZ_TABLE })] }));
  return new TableCell({ verticalAlign: VerticalAlign.CENTER, margins: { top: 40, bottom: 40, left: 80, right: 60 }, children: kids });
}
...
function buildSummaryTableDocx(input) {
  ...
  const children = [];
  // Top □ line: 단락 앞 0pt, 단락 뒤 10pt.
  children.push(sectionHeader(groupLabel, candidates.length + "名", { before: 0, after: 200 }));
  ...
  const headerCell = (label) =>
    new TableCell({
      ...
      borders: { top: thin, bottom: thick, left: thin, right: thin },
      children: [new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 0, line: LINE_TIGHT, lineRule: "auto" }, children: [run(label, { size: SZ_TABLE, bold: true, scale: SUM_SCALE })] })]
    });
  ...
  children.push(new Table({
    width: { size: SUM_NAME_W + SUM_MID_W + SUM_COMP_W, type: WidthType.DXA },
    layout: TableLayoutType.FIXED,
    columnWidths: [SUM_NAME_W, SUM_MID_W, SUM_COMP_W],
