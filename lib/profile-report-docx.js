...
function competencyParagraphs(competencies) {
  ...
  blocks.forEach((block, bi) => {
    ...
    bullets.forEach((b, i) => {
      ...
      // · bullets are meant to fill TWO lines, so they are NOT condensed to one
      // line. Bullets within the same block sit tight together (0pt after); a
      // 12pt gap separates blocks and 6pt trails the final block's last bullet.
      const after = lastBeforeNextBlock ? 240 : isLastBullet ? AF_BULLET : 0;
      out.push(bodyPara([run(fullB, { size: SZ_BULLET })], { after }));
