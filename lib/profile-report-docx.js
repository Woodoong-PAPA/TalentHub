...
const MARGIN_SIDE = 1300; // twips each side
...
// Borderless, no-fill Simple Text Box (VML t202). Floats over the text and is
// draggable in Word. NOTE: adding CSS position/z-index to the VML style trips
// Word's "unreadable content" check, so it is intentionally omitted; the
// borderless "in front of text" behavior is applied via post-processing.
...
function photoParagraph(photo, align) {
  return new Paragraph({
    ...
    children: [
      new ImageRun({ data: photo.buffer, transformation: { width: photo.width || 84, height: photo.height || 104 } })
    ]
  });
}
...
function summaryNameCell(data) {
  ...
  if (data.photo && data.photo.buffer) kids.push(photoParagraph({ buffer: data.photo.buffer, width: data.photo.width || 60, height: data.photo.height || 74 }, AlignmentType.CENTER));
  kids.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 0, line: LINE_TIGHT, lineRule: "auto" }, children: [run(data.name || "", { size: SZ_TABLE, bold: true })] }));
  if (data.birthLine) kids.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 0, line: LINE_TIGHT, lineRule: "auto" }, children: [run("(" + data.birthLine + ")", { size: SZ_TABLE_SMALL })] }));
  return new TableCell({ verticalAlign: VerticalAlign.CENTER, margins: { top: 40, bottom: 40, left: 40, right: 40 }, children: kids });
}
...
async function packDoc(children) {
  const doc = new Document({
    ...
    sections: [{ properties: { page: { margin: { top: 1100, bottom: 1000, left: MARGIN_SIDE, right: MARGIN_SIDE } } }, children }]
  });
