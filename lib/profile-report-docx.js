...
// A gray annotation line placed just under its reference line. Rendered as
// an ordinary paragraph (reliably valid in Word) with gray text. See
// buildAnnotationTextbox() for the optional movable text-box variant.
function footnoteBox(text, opts = {}) {
  return new Paragraph({
    spacing: { after: AF_SET, line: LINE, lineRule: "auto" },
    indent: { left: opts.indent == null ? 560 : opts.indent },
    children: [run(text, { size: opts.size || SZ_NOTE, color: GRAY })]
  });
}

function pushNotes(target, notes, indent) {
  (notes || []).forEach((n) => target.push(footnoteBox(n, { indent })));
}

// OPTIONAL: a borderless gray "Simple Text Box" (VML t202) the recruiter can
// drag under the exact word in Word. Kept separate and OFF by default because
// hand-injected VML can trip Word's "unreadable content" check on some setups;
// enable only after confirming it opens in the target Word version.
let fnSeq = 0;
function buildAnnotationTextbox(text, opts = {}) {
