...
function competencyParagraphs(competencies) {
  ...
  blocks.forEach((block, bi) => {
    ...
    bullets.forEach((b, i) => {
      ...
      const lastBeforeNextBlock = i === bullets.length - 1 && bi < blocks.length - 1;
      out.push(bodyPara([run(fullB, { size: SZ_BULLET, characterSpacing: condenseFor(fullB, SZ_BULLET) })], { after: lastBeforeNextBlock ? 240 : AF_BULLET }));
