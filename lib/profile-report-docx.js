...
//   • Annotations render as standard visible Simple Text Boxes (VML t202,
//     white fill + thin outline) floating IN FRONT OF text (positive z-index),
//     so they are selectable/movable in Word. Source citations are not
...
function sectionHeader(label, trailing) {
  ...
  return new Paragraph({ children: kids, spacing: { before: 240, after: AF_HEADER, line: LINE, lineRule: "auto" } });
}

// Annotation rendering has two variants, chosen per request:
//   • "textbox" (PC, default) — a visible floating VML Simple Text Box (t202),
//     draggable/editable in Word. The white fill + thin outline are applied via
//     post-processing (docx exposes no stroke/fill option on Textbox).
...
function competencyParagraphs(competencies) {
  ...
  blocks.forEach((block, bi) => {
    ...
    bullets.forEach((b, i) => {
      ...
      // The last bullet before the next "- headline" gets a 12pt gap.
      const lastBeforeNextBlock = isLastBullet && bi < blocks.length - 1;
      // · bullets fill TWO lines, so they are NOT condensed to one line. The
      // bullet's own two wrapped lines hug (single line spacing = 첫줄 뒷단락
      // 0pt). Between bullets: 0pt within a block; 12pt before the next block;
      // 6pt after the final block's last bullet.
      const after = lastBeforeNextBlock ? 240 : isLastBullet ? AF_BULLET : 0;
      out.push(new Paragraph({ children: [run(fullB, { size: SZ_BULLET })], spacing: { after, line: LINE_TIGHT, lineRule: "auto" } }));
...
function educationLines(education, opts = {}) {
  ...
  list.forEach((e, i) => {
    ...
    out.push(new Paragraph({ spacing: { after: e.note ? 0 : isLast ? lastAfter : ROW_AFTER, line: LINE_TIGHT, lineRule: "auto" }, tabStops: [{ type: TabStopType.RIGHT, position: TabStopPosition.MAX }], children: parts }));
...
function careerLines(career, opts = {}) {
  ...
  list.forEach((c, i) => {
    ...
    const isLast = i === list.length - 1;
    out.push(new Paragraph({ spacing: { after: c.note ? 0 : isLast ? lastAfter : ROW_AFTER, line: LINE_TIGHT, lineRule: "auto" }, tabStops: [{ type: TabStopType.RIGHT, position: TabStopPosition.MAX }], children: head }));
...
async function makeTextboxesSimple(buffer) {
  ...
  // Emit each annotation as a standard Word "Simple Text Box" (단순형): white
  // fill + a thin light-gray outline, floating in front of the text. A visible,
  // solid box is what "삽입 > 텍스트 상자 > 단순형" produces and, unlike the old
  // invisible (no-fill, no-border) shape, it can actually be clicked, moved and
  // edited in Word.
  xml = xml.replace(
    /<v:shape (?![^>]*\bstroked=)/g,
    '<v:shape stroked="t" strokecolor="#BFBFBF" strokeweight=".75pt" filled="t" fillcolor="#FFFFFF" '
  );
