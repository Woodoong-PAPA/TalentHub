...
function splitBulletLines(text, sizeHalf) {
  ...
  const cont = "      "; // 6 spaces so line 2 aligns under the text
  ...
  blocks.forEach((block, bi) => {
    ...
    bullets.forEach((b, i) => {
      ...
      const mk = (txt, after, noteList) => {
        const kids = [run(txt, { size: bulletSize })];
...
function educationLines(education, opts = {}) {
  ...
  list.forEach((e, i) => {
    ...
    const parts = [run((e.degree || "") + ") ", { size: sz, bold: true, scale }), run(cleanText(e.school), { size: sz, bold: true, scale })];
    if (e.major) parts.push(run(", " + cleanText(e.major), { size: sz, scale }));
    if (e.year) {
      parts.push(new TextRun({ text: "\t", font: { name: FONT } }));
      parts.push(run("('" + String(e.year).replace(/^'/, "") + ")", { size: sz, scale }));
    }
...
function careerLines(career, opts = {}) {
  ...
  list.forEach((c, i) => {
    ...
    const head = [run("- ", { size: sz, scale })];
    if (c.country) head.push(run(c.country, { size: sz, bold: true, scale }));
    head.push(run(cleanText(c.org), { size: sz, bold: true, scale }));
    if (c.title) head.push(run(", " + cleanText(c.title), { size: sz, scale }));
    if (c.period) {
      head.push(new TextRun({ text: "\t", font: { name: FONT } }));
      head.push(run("(" + c.period + ")", { size: sz, scale }));
    }
    const isLast = i === list.length - 1;
...
function personalBlockB(data) {
  const left = [];
  // 연령/학력 at 13pt bold, spaced labels, 12pt before/after (sample style).
  if (data.age || data.birthMark) {
    const ageText = (data.age ? data.age + "세" : "") + (data.birthMark ? " (" + data.birthMark + ")" : "");
    left.push(new Paragraph({ spacing: { before: 240, after: 240, line: LINE, lineRule: "auto" }, children: [run("   - 연  령 : ", { size: SZ_DASH, bold: true }), run(ageText.trim(), { size: SZ_DASH, bold: true })] }));
  }
  const edu = expandEducation(data.education);
  edu.forEach((e, i) => {
    const parts = [run(i === 0 ? "   - 학  력 : " : "              ", { size: SZ_DASH, bold: true })];
    parts.push(run((e.degree || "") + ") " + cleanText(e.school) + (e.major ? ", " + cleanText(e.major) : ""), { size: SZ_DASH, bold: true }));
    if (e.year) {
      parts.push(new TextRun({ text: "\t", font: { name: FONT } }));
      parts.push(run("('" + String(e.year).replace(/^'/, "") + ")", { size: SZ_DASH, bold: true }));
    }
    const note = e.note && noteAllowed(e.note) ? e.note : null;
    if (note && !INLINE_NOTES) parts.push(textboxNote(note, { width: 200 }));
    const isLast = i === edu.length - 1;
    left.push(new Paragraph({ spacing: { before: i === 0 ? 240 : 0, after: isLast ? 240 : 0, line: LINE, lineRule: "auto" }, tabStops: [{ type: TabStopType.LEFT, position: 3400 }], children: parts }));
    if (note && INLINE_NOTES) left.push(inlineNote(note, { width: 200 }));
  });
...
function careerLinesB(career) {
  // 주요 사항: 14pt bold, period right-tabbed, 12pt before/after (sample style).
  const out = [];
  const list = normalizeCareer(career);
  list.forEach((c, i) => {
    const sz = SZ_HEADER; // 14pt
    ...
    const note = c.note && noteAllowed(c.note) ? c.note : null;
    if (note && !INLINE_NOTES) head.push(textboxNote(note, { width: 240 }));
    const isLast = i === list.length - 1;
    out.push(new Paragraph({ spacing: { before: 240, after: isLast ? 320 : 240, line: LINE, lineRule: "auto" }, tabStops: [{ type: TabStopType.RIGHT, position: TabStopPosition.MAX }], children: head }));
    if (note && INLINE_NOTES) out.push(inlineNote(note, { width: 240 }));
  });
  return out;
}
...
function buildBasicProfileDocx(data) {
  ...
  children.push(sectionHeader("주요 사항", null, { before: 240, after: 240 }));
  careerLinesB(data.career).forEach((p) => children.push(p));

  children.push(sectionHeader("참고 사항", null, { before: 240, after: 240 }));
  // Basic 참고사항: 14pt bold headlines (no underline) + 14pt two-line bullets.
  competencyParagraphs(data.competencies, { headSize: SZ_HEADER, bulletSize: SZ_HEADER, underline: false }).forEach((p) => children.push(p));
  (data.competencyNotes || []).forEach((n) => {
    const obj = typeof n === "string" ? { text: n } : n;
    const t = "※ " + cleanText(obj.text);
