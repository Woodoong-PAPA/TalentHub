...
- 이름, 현재/최근 회사, 직무, 주요 역량/성과, 학력, 경력 값을 자동 입력한다.
- 출생년도, 이메일 주소, 휴대폰 번호, 링크드인 주소, 기타 참고 URL이 이력서에 있으면 자동 입력한다.
- 이력서 파일 안에 프로필 사진 후보가 있으면 얼굴 또는 증명사진 영역을 크롭해 프로필 사진 미리보기에 자동 반영한다.
...
- 얼굴 탐지가 불가능한 PDF 이미지를 무리하게 프로필 사진으로 등록하지 않는다.
...
flowchart TD
    ...
    J --> L["필드 신뢰도 계산"]
    L --> M["등록 폼 자동 입력"]
    P --> Q["얼굴 탐지 또는 프로필형 이미지 크롭"]
    Q --> R["프로필 사진 미리보기 자동 반영"]
    M --> N["사용자 검수 후 직접 등록"]
```
