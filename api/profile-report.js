...
const FORMAT_GUIDES = {
  interview: [
    "형식 A(면접용 상세): 제목은 【 이름 직급(소속) Profile 】 형태다. role에 직급/직책(예: '교수','상무','대표')을, orgShort에 소속 약칭을 채운다. 학교와 회사를 함께 대표해야 하면 orgShort를 'MIT/Meta'처럼 슬래시로 묶는다.",
    "결과는 반드시 A4 1장 이내여야 하므로 분량을 엄격히 지킨다.",
    "competencyTitle에 '전문 역량' 한 줄 요약을 쓴다(예: '생체모사 로보틱스·휴머노이드 R&D 전문가').",
    "competencies는 '정확히 2개'의 밑줄 헤드라인(headline)으로 구성한다. headline은 한 줄짜리 핵심 요약(32~40자), 각 headline의 bullets는 정확히 2개이며 각 bullet은 두 줄 분량(46~64자)의 구체 근거 문장이다.",
    "각 bullet에는 뒷받침 태그(bullets.notes)를 0~2개 붙인다(기관명·지표·약어). 각 경력(career)에는 핵심 역할 태그(career.note)를 가능하면 붙인다.",
